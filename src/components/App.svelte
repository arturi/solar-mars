...
<style>
  main {
    font-family: 'Avenir Next', Avenir, '-apple-system', Helvetica, Arial,
      sans-serif;
    max-width: 650px;
    padding: 0 15px;
    margin: auto;
  ...
  {#each allEntries as entry}
    <article>
      <header>
        {#if entry.feed.title}
          <h2>
            <a href={entry.feed.link}>{entry.feed.title}</a>
          </h2>
        {/if}
        {#if entry.title}
          <h1>
      ...
      <date>
        <a class="pink" href={entry.link}>
          → {timeAgo(entry.pubDate)} ({unixToHumanDate(entry.pubDate)})
        </a>
      </date>
    </article>
  {:else}Loading...{/each}
